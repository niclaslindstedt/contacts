 ...
 * delegates straight through — clearing, capturing, and subscribing all act on
 * the underlying store.
 *
 * Reversal rather than a sort on `ts`: the buffer is strictly append-ordered by
 * wall clock, so reversing is exact and also puts the newest of several entries
 * sharing a millisecond first, which a stable sort by `ts` would not.
 */
