...
// Politeness for the media byte transports (`photoFileStore.ts`, and through it
// the photo and attachment externalisers).
//
// The document itself is one small request per save, so the framework's storage
// adapters can be blunt about it. Media is the opposite shape: opening an
// address book with a hundred photographed contacts wants *hundreds* of content
// -API round-trips, and the naive `Promise.all` that used to drive them fired
// every one at once. Two things go wrong at that fan-out:
//
//   - **The provider throttles.** Dropbox answers `429 Too Many Requests` with a
//     `Retry-After`, which nothing in the byte transport used to read — the
//     request simply failed and the photo went missing.
...
//
// So this module supplies the two things that make a media sweep survive a real
...
// transient failure instead of reporting the photo as unreadable.
...
/** How many media files one sweep keeps in flight at once. Low on purpose: the
 ...
 *  throughput and is what turns a large address book into a 429 storm. */
export const MEDIA_CONCURRENCY = 4;

/** Attempts (the first try plus its retries) any one media op gets. */
