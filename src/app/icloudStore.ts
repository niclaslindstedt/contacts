...
// byte store satisfies the same `PhotoFileStore` contract the Dropbox, Drive
// and folder transports do, so the photo / attachment externalisers and the
// dated backups all drive it unchanged.
//
// Everything is filed under the container's document root, so what lands in
// iCloud Drive is a browsable tree — `contacts-<slug>.json` with `photos/`,
// `attachments/` and `backups/` beside it — that the reader can open in the
// Files app on any of their devices.
//
// WHAT THIS LAYER OWNS is one conversion and one policy:
//
//   • BASE64. The bridge to a host is a string channel, so image and document
//     bytes cross as base64 and are decoded here. Text does not: the document
//     is JSON and crosses as itself.
...
/** The backend's stable identifier, used to key its local cache and to label
 *  the adapter.
 *
 *  The framework's `StorageBackendId` is the closed union of the four backends
 *  it ships, while its own documentation says an app that adds a backend
 *  "keys it under its own string" — so the cast is the type catching up with
 *  the contract, not a hole in it. Spelled once, here. */
...
/** The whole-document adapter: `contacts-<slug>.json` in the container's
 *  document root, beside the `photos/` and `attachments/` trees. */
...
/** The byte-level {@link PhotoFileStore} the photo / attachment / backup
 *  layers drive — the same contract the Dropbox, Drive and folder transports
 *  satisfy, so all four are interchangeable.
 *
 ...
 *  callers compose already end in `.jpg` / `.pdf` / `.zip`. */
export function icloudPhotoFileStore(host: ICloudHost): PhotoFileStore {
  return {
    async list() {
...
// rather than spread in one call: a kept photo original runs to a few million
