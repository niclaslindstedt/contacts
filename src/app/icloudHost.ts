...
// document's name, the deterministic `photos/` and `attachments/` layout, what
// a conflict means and when a save is due are all decided in `useSyncEngine.ts`
// against the framework's storage adapters — exactly as they are for a picked
// local folder. A host that knew what a contact was would be a second
 ...
 *  holding a subscription to the reader's address book for as long as the app
 *  is open.
 *
 *  Text and bytes are separate pairs because the document is JSON and a photo
...
export const ICLOUD_HOST_EVENT = "contacts:icloud-host";
 ...
 *  the reader every photo the sweep would otherwise have found. */
