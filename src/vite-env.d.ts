...
interface ImportMetaEnv {
  // Boot straight into a developer in-memory backend, seeded with sample
  ...
  // loads a big stress-test spread; `demo` loads the presentation-grade demo
  // address book (`src/app/dev/demoData.ts`). Unset (or `0`) starts on the
  // real address book. `npm run dev` sets this to `large` by default.
