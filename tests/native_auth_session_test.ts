...
const REDIRECT = redirectUriFor("se.agilator.contacts");
...
describe("the redirect URI", () => {
  it("is the app's scheme with an oauth path — the string Dropbox must list", () => {
    expect(REDIRECT).toBe("se.agilator.contacts://oauth");
  });

  it("has the bundle id for its scheme, so the store build returns on se.agilator.contacts://oauth", () => {
    // RFC 8252 §7.1: a reverse-DNS scheme no other app can claim. It follows
    // APP_BUNDLE_ID rather than being committed.
    const scheme = schemeFor("se.agilator.contacts");
    expect(scheme).toBe("se.agilator.contacts");
    expect(redirectUriFor(scheme as string)).toBe(REDIRECT);
  });

  it("falls back with the bundle id in a plain checkout", () => {
    expect(schemeFor(undefined)).toBe("dev.local.contacts");
  });
});
...
describe("isAuthSessionRequest", () => {
  ...
  it("ignores messages that are not ours", () => {
    expect(
      isAuthSessionRequest({ ...good, type: "contacts-native/report" }),
    ).toBe(false);
    expect(
      isAuthSessionRequest({ ...good, type: "contacts-native/icloud-request" }),
    ).toBe(false);
...
describe("authSessionResolveScript", () => {
  it("does not let a provider-controlled URL break out of the script", () => {
    ...
    const nasty = `se.agilator.contacts://oauth?code=");alert(1);//&x=${String.fromCharCode(0x2028)}`;
