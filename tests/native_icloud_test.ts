...
describe("the injected provider", () => {
  it("installs itself where the web app looks for it", () => {
    ...
    // `window.__contactsICloud`; a rename on either side is not an error, it
    // is a backend that never appears.
    expect(ICLOUD_SCRIPT).toContain("window.__contactsICloud");
  });
  ...
  it("offers every method the seam validates, at the version it knows", () => {
    ...
    globals.window = { __contactsICloud: host };
    try {
...
describe("isICloudRequest", () => {
  ...
  it("ignores anything the page posts that is not ours", () => {
    ...
    expect(isICloudRequest({ ...good, type: "contacts-native/report" })).toBe(
      false,
    );
...
function runScript(script: string): { id: string; result: unknown } | null {
  ...
  const stub = {
    __contactsICloudResolve: (id: string, result: unknown) => {
      seen = { id, result };
    },
  };
...
describe("resolveScript", () => {
  it("carries the answer back as parsed JSON, not as a literal", () => {
    ...
    expect(script).toContain("window.__contactsICloudResolve");
  });

  it("does not let a contact's own text break out of the script", () => {
    // The payload is somebody's address book, and a name is arbitrary user
    // text. This is the case that turns a quote in a surname into injected
