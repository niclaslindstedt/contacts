...
function withInstalled<T>(value: unknown, body: () => T): T {
  ...
  globals.window = { __contactsICloud: value };
  try {
    return body();
  } finally {
...
describe("parseICloudEntries", () => {
  it("keeps the path and the revision token", () => {
    expect(
      parseICloudEntries([{ path: "contacts-default.json", rev: "17-42" }]),
    ).toEqual([{ path: "contacts-default.json", rev: "17-42" }]);
  });
  ...
  it("keeps an entry with no revision", () => {
    ...
    expect(parseICloudEntries([{ path: "photos/a.jpg" }])).toEqual([
      { path: "photos/a.jpg" },
    ]);
  });

  it("drops malformed rows one at a time", () => {
    // One bad row must not cost the reader every photo the sweep would
    ...
    expect(
      parseICloudEntries([
        ...
        null,
        "photos/b.jpg",
        { path: "photos/c.jpg", rev: 7 },
      ]),
    ).toEqual([{ path: "photos/a.jpg" }, { path: "photos/c.jpg" }]);
  });
