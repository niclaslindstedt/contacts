...
// byte of a photo and every byte of the document goes through them, and the
// one conversion they own — base64, because the bridge to a host is a string
// channel — is the kind that works on a smiley and fails on a JPEG.
...
describe("base64", () => {
  it("round-trips arbitrary bytes", () => {
    ...
    // here rather than as a corrupt photo on somebody's phone.
  ...
  it("round-trips a payload larger than one chunk", () => {
    ...
    // into one `fromCharCode` call, because a kept photo original overflows
...
describe("the document store", () => {
  it("lists what the host holds, with revisions", async () => {
    const { host } = fakeHost({
      "contacts-default.json": new TextEncoder().encode("{}"),
    });
    await expect(icloudFileStore(host).list()).resolves.toEqual([
      { path: "contacts-default.json", rev: "21" },
    ]);
  });
  ...
  it("reads a missing file as nothing, not as an error", async () => {
    ...
    const { host } = fakeHost();
    await expect(icloudFileStore(host).read("contacts-x.json")).resolves.toBe(
      null,
    );
  });
  ...
  it("writes text through unchanged", async () => {
    ...
    // Non-ASCII on purpose: a name is arbitrary user text.
    await store.write("contacts-default.json", '{"name":"Åsa Ödegård"}');
    expect(await store.read("contacts-default.json")).toBe(
      '{"name":"Åsa Ödegård"}',
    );
    expect(files.has("contacts-default.json")).toBe(true);
  });
...
describe("the byte store", () => {
  it("round-trips image bytes verbatim", async () => {
    const { host } = fakeHost();
    const store = icloudPhotoFileStore(host);
    // A JPEG's first bytes — the ones a text-mode transport mangles.
    const jpeg = new Uint8Array([0xff, 0xd8, 0xff, 0xe0, 0x00, 0x10, 0x4a]);
    await store.write("photos/asa-home-1.jpg", jpeg);
    expect([...((await store.read("photos/asa-home-1.jpg")) ?? [])]).toEqual([
      ...jpeg,
    ]);
  });
  ...
  it("reads a missing file as nothing", async () => {
    const { host } = fakeHost();
    await expect(
      icloudPhotoFileStore(host).read("photos/gone.jpg"),
    ).resolves.toBe(null);
  });

  it("lists paths, so the photo and prune sweeps can scope themselves", async () => {
    const { host } = fakeHost({
      "contacts-default.json": new Uint8Array([1]),
      "photos/a.jpg": new Uint8Array([2]),
      "attachments/b.pdf": new Uint8Array([3]),
    });
    await expect(icloudPhotoFileStore(host).list()).resolves.toEqual([
