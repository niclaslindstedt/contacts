...
Contacts runs in any browser, and it installs to a home screen or a dock as a
...
around it: the same cards and folders, the same settings, the same export. Two differences are worth knowing about.
...
the contacts you keep in Chrome are not the ones the desktop app opens. Connect the same folder or Dropbox in both (Settings → **Storage**) and they share one address book, or carry it across as a vCard or JSON export.

Uninstalling the app removes that storage with it, so export anything you want to keep first.
