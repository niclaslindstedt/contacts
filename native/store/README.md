...
Everything App Store Connect needs for Contacts' product page, compiled from one
...
Contacts ships on the **App Store** alone — iPhone and iPad. `listing.mts`
...
Taken of **the real app on its demo data**: `VITE_SEED=demo` builds Contacts
onto one person's address book held in memory (`src/app/dev/demoData.ts`;
