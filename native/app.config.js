...
// has no version of its own — it ships one build of the contacts app, and the
...
module.exports = () => ({
  expo: {
    ...
    // is measured for, so the app is pinned the way the reader holds the
    // phone.
    ...
    // se.agilator.contacts in production, dev.local.contacts in a plain
    ...
    // is the redirect URI the Dropbox app must list (RELEASING.md).
    scheme: BUNDLE_ID,
    ...
    plugins: [
      ...
      // synced address book shows up under "Contacts" in the Files app rather
    ...
    extra: {
      ...
      // contacts app bundled inside it (assets/webroot.zip) from a loopback
      ...
      // a debug build at a deployed slot, set EXPO_PUBLIC_CONTACTS_URL at
