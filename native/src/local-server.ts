...
//
// Adapted from the sibling `calendar` repo's wrapper, which is where this
// approach is proven. Two decisions carry the whole design:
//
...
// store: every contact the user has entered would appear to vanish. The port
// is therefore pinned, and the ladder below only exists so that something else
...
// Security blocks `http://127.0.0.1:…` from `WKWebView` while
// `http://localhost:…` is permitted, even with exception domains declared for
// both. The failure mode is a silent blank page on iOS.
//
