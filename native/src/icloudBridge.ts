...
//
//   • a script that defines `window.__contactsICloud`, whose methods post a
...
//
// The wrapper decides nothing about the address book. It moves opaque files
// between the page and a folder in iCloud Drive: which file the document lives
// in, how photos are filed beside it, what a conflict means and when a save is
// due are all the web app's, in `src/app/useSyncEngine.ts` against the
// framework's storage adapters. A wrapper that parsed a contact would be a
// second implementation of the app's domain.
//
...
export const ICLOUD_REQUEST_TYPE = "contacts-native/icloud-request";
 ...
 *  crosses as text; a photo or an attachment is binary and crosses as base64,
...
export const ICLOUD_SCRIPT = `(function () {
  ...
  window.__contactsICloudResolve = function (id, result) {
 ...
 * address book: a contact's name is arbitrary user text, and text is exactly
...
export function resolveScript(
  ...
  return `(function () {
    try {
      ...
      if (window.__contactsICloudResolve) {
        window.__contactsICloudResolve(answer.id, answer.result);
      }
    } catch (e) {}
  })(); true;`;
}
