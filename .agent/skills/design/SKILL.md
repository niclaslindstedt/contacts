...
| Helper                         | What it does                                                                                                                                                                                                                                                                                                |
| ------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `openApp(page)`                | Navigate to the app root and wait until the shell has rendered (the List heading — the desktop landing state — or the card's Edit/Done control, or the phone's "Open sidebar" button). Universal first step.                                                                                                |
| `toggleRow(page, name)`        | Flip a ToggleRow / checkbox by accessible name ("Company contact", "Emergency contact"). Forces the click through — the framework checkbox input is sr-only behind its painted glyph, so a normal click is intercepted.                                                                                     |
...
| `openMassEdit(page)`           | Open the List page, enter select mode, tick everything via the "Select all" row, and open the bulk-edit ("Edit selected") modal from the header pencil (waits on the modal's "Edit N contacts" title — the Section titles are aria-labelled groups, not headings).                                          |
