...
impl Site {
    fn new(name: &str) -> Self {
        let root = std::env::temp_dir().join(format!("contacts-webroot-{name}"));
...
fn a_symlink_out_of_the_webroot_is_refused() {
    ...
    let outside = std::env::temp_dir().join("contacts-webroot-symlink-outside.txt");
