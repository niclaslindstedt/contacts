...
//! wrong to simplify.
//!
//! The site is served from a registered private scheme (`contacts://`, see
...
//! to lose:
//!
//!  1. **One stable origin.** The user's contacts live in IndexedDB and their
//!     settings in `localStorage`, both keyed by origin. `contacts://` is a
//!     constant, so the contacts survive every update. A `file://` page (or a
//!     server on an ephemeral port) would hand the user a different origin —
//!     and an empty contacts — at some point.
//!
...
/// `contacts:///C:/…` must not become a drive letter.
