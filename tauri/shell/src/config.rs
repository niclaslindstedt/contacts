...
//! shipping: `CONTACTS_APP_URL` points the window at a remote URL instead (the
//! `/preview/` deploy slot, say), and `CONTACTS_WEBROOT` serves a different
...
/// IndexedDB unable to keep a contact between launches. A registered scheme
/// gives one stable origin that the contacts are keyed to for the life of the
/// install.
pub const APP_SCHEME: &str = "contacts";
...
/// CONSTANT — the origin is what the user's contacts are keyed to, so changing
...
pub const WINDOW_TITLE: &str = "Contacts";

/// The window's opening size, and the smallest it may be dragged to.
///
/// The floor is the narrowest the list and an open card are both usable at —
