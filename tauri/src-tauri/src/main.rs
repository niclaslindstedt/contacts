...
//! The Contacts desktop shell — a window showing the bundled site, and nothing
//! else.
//!
...
//! here needed no change to the app at all. Everything Contacts does — the
...
use contacts_shell::config::APP_SCHEME;
use contacts_shell::oauth::{AWAIT_PATH, BEGIN_PATH};
use contacts_shell::webroot::webroot_exists;
...
fn main() {
    tauri::Builder::default()
        ...
        // `contacts_shell::webroot` for the three properties this arrangement is
        ...
        // `contacts_shell::oauth`), which can be minutes, so it answers from a
        ...
        .setup(|app| {
            ...
            if contacts_shell::config::remote_app_url().is_none() && !webroot_exists(&root) {
                let message = format!(
                    "Contacts could not find the app it is meant to show.\n\n\
                     Looked in: {}\n\n\
        ...
        .expect("the Contacts desktop shell could not start");
}
