...
//! Every decision about that file is `contacts_shell::window_state`'s; this
//! file only asks the window and the monitors.

use contacts_shell::config::{
    app_origin, is_internal_url, remote_app_url, start_url, BRAND_BG, WINDOW_TITLE,
};
use contacts_shell::window_state::{
...
const DEFAULT_WINDOW: WindowState = contacts_shell::window_state::DEFAULT_STATE;
