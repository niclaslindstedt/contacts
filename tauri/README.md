...
A desktop wrapper around Contacts for **Windows, macOS and Linux**. It is a thin
...
minimum. Everything Contacts does — the document, the settings, the sync engine —
...
| `src-tauri/src/protocol.rs` | Answering `contacts://` off the bundled `webroot/`            |
...
`cargo test -p contacts-shell` runs the entire decision layer on a machine with a
...
**The origin is the one thing to be careful with.** The user's contacts live in
...
`file://` page is an opaque origin, which would leave the contacts unable to
survive a launch.
...
| Variable           | Effect                                                               |
| ------------------ | -------------------------------------------------------------------- |
| `CONTACTS_APP_URL` | Load a remote URL instead of the bundled site (the `/preview/` slot) |
| `CONTACTS_WEBROOT` | Serve the site from somewhere else without rebuilding                |
...
identifier (`dev.local.contacts`). What an installed copy is called and the
